...
//CONFIG - TSMPoke Configuration
const BUILD_DIR = 'build',
      RELEASE_DIR = 'release',
      BOT_DIR = `${BUILD_DIR}/tsmpoke`,
...
gulp.task('python:install', callback => {
    async.waterfall([
        ...
        cb => async.concat(fs.readFileSync('build/tsmpoke/requirements.txt')
            .toString()
            .split('\n')
...
gulp.task('python:associate', callback => {
    async.waterfall([
        _ => fs.readFile(`${BOT_DIR}/tsmpoke_cli.py`, _),
        (data, cb) => {
        ...
        (data, _) => fs.writeFile(`${BOT_DIR}/tsmpoke_cli.py`, data, _)
    ], callback);
});
...
gulp.task('electron:osx', () => {
    return gulp.src(PACKAGE_SRC, {base: 'build'})
        .pipe(packageElectron({
            version: '1.3.3',
            platform: 'darwin',
            darwinIcon: 'src/assets/resources/image/icons/tsmpoke.icns',
            darwinBundleIdentifier: 'com.github.tsmpoke'
...
gulp.task('electron:windows', () => {
    return gulp.src(PACKAGE_SRC.concat(`${BUILD_DIR}pywin/**/*`), {base: 'build'})
        .pipe(packageElectron({
            ...
            platform: 'win32',
            arch: 'ia32',
            winIcon: 'src/assets/resources/image/icons/tsmpoke.ico',
            companyName: 'TSMPoke',
            copyright: '2024 TSMPoke, All Rights Reserved.'
        }))
...
gulp.task('tsmpoke:update', (callback) => {
    async.series([
...
gulp.task('tsmpoke:prune', (callback) => {
...
gulp.task('tsmpoke', _ => rseq('tsmpoke:update', ['tsmpoke:prune', 'python'], _));

gulp.task('clean', () => vfs.src([`${BUILD_DIR}/*`, `!${BUILD_DIR}/{tsmpoke,tsmpoke/*,pywin,pywin/*}`]).pipe(grimraf()));
...
gulp.task('build', _ => rseq('clean', ['tsmpoke', 'build:node', 'build:src'], _));
